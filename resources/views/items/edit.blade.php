<x-app-layout>
    ...
    <div class="max-w-[1410px] mx-auto flex gap-x-20 my-20 ">
        ...
        <form method="post" action="{{ action([\App\Http\Controllers\ItemController::class,'update'],$item) }}"
            ...
            <div class="flex flex-col gap-y-5">
                ...
                <x-text-input value="{{ $item->price?? old('price') }}" name="price"
                              placeholder="Enter price for one item (ETH)"/>
            </div>
            <div class="flex flex-col gap-y-5">
                <x-input-label value="Title"/>
                <x-text-input value="{{ $item->title?? old('title') }}" name="title" placeholder="Item Name"/>
            </div>
            <div class="flex flex-col gap-y-5">
            ...
            <div class="flex gap-x-5">
                <div class="flex flex-col gap-y-5 flex-1">
                    ...
                    <x-text-input value="{{ $item->royalties?? old('royalties') }}" type="number" step="0.01"
                                  name="royalties"
                                  placeholder="5%"/>
                </div>
                <div class="flex flex-col gap-y-5 flex-1">
            ...
            <div class="flex gap-x-5">
                ...
                <div class="flex flex-col gap-y-5 flex-1">
                    ...
                    <x-select-input :options="$item->author->collections"
                                    name="collection_id"/>
                </div>
            </div>
